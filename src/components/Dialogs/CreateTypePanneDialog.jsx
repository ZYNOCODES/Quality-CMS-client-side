...
export default function CreateTypePanneDialog(props) {
    ...
    const handleOnCreate = async (event) => {
        ...
        if (confirmation) {
            try {
                setLoading(true);
                const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/pannetypeassignment/create/${props.code}`, 
                    {
                        typePanne: Type,
                        agent: props.agent
                    },
                ...
                if (error.response) {
                    ...
                    console.error("Error creating type de panne: No response received");
                } else {
                    // Something happened in setting up the request that triggered an Error
                    console.error("Error creating type de panne", error);
                }
            }
            setconfirmation(false);
            setType('');
    ...
    return (
        ...
        >
            {!loading && 
                <>
                    <DialogTitle>Creation d'un type de panne</DialogTitle>
                    <DialogContent>
                        <DialogContentText>
