...
export default function ActionDialog(props) {
  ...
  const onUploadXLSXFile = async (formData) => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/xlsx/upload/pannetype`, 
        formData, 
        {
          headers: {
      ...
      if (error.response) {
        ...
        // Request was made but no response was received
        console.error("Error importing panne types: No response received");
      } else {
        // Something happened in setting up the request that triggered an Error
        console.error("Error importing panne types", error);
      }
    }
  }
  ...
  >
      ...
      <ConfirmationUploadExcelDialog
          ...
          name="d'insertion des types de panne à partir d'un fichier Excel"
      ...
      <NonSavedXLSXDisplayerDialog
          ...
          name="Les types de panne non sauvegardés"
          loading={submitionLoading}
