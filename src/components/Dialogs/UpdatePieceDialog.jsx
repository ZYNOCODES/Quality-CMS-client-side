...
export default function UpdatePieceDialog(props) {
    ...
    const handleOnUpdate = async (event) => {
        ...
        if (confirmation) {
            try {
                setLoading(true);
                const response = await axios.patch(import.meta.env.VITE_APP_URL_BASE+`/piece/${props.code}`, 
                    {
                        name: Name
                    },
                    {
                ...
                if (error.response) {
                    ...
                    console.error("Error updating piece: No response received");
                } else {
