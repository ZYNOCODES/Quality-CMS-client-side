...
export default function Accessorydialog(props) {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(process.env.REACT_APP_URL_BASE+`/accessoire/create`, 
        { 
          Nom: Nom,
          Quantite: Quantite
        }, 
      ...
      if (error.response) {
        ...
        console.error("Error creating Accessory: No response received", error.request);
      } else {
          ...
          <DialogContent sx={{ marginTop: '40px' }}> 
            <Box display="flex" flexDirection="column" alignItems="flex-start" mt={2} sx={{ width: '100%' }}>
                  <TextFieldComponent 
                      type="text" 
                      label="Nom" 
                      initialHelperText="Entrer le nom du accessoire" 
                      minLength={0} 
                      maxLength={15}
                  ...
                  <TextFieldComponent 
                      type="text" 
                      label="Quantité" 
                      initialHelperText="Entre la quantité de accessoire" 
                      minLength={0} 
                      maxLength={15} 
