...
export default function LotDialog(props) {
  ...
  const onUploadXLSXFile = async (formData) => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/xlsx/upload/lot`, 
        formData, 
        {
          headers: {
      ...
      if (error.response) {
        ...
        // Request was made but no response was received
        console.error("Error importing Lots: No response received");
      } else {
        // Something happened in setting up the request that triggered an Error
  ...
  >
      ...
      <ConfirmationUploadExcelDialog
        ...
        name="d'insertion des lots à partir d'un fichier Excel"
