...
export default function PieceDialog(props) {
  ...
  const onUploadXLSXFile = async (formData) => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/xlsx/upload/piece`, 
        formData, 
        {
          headers: {
      ...
      if (error.response) {
        ...
        // Request was made but no response was received
        console.error("Error importing pieces: No response received");
      } else {
        // Something happened in setting up the request that triggered an Error
  ...
  >
      ...
      <ConfirmationUploadExcelDialog
          ...
          name="d'insertion des pieces à partir d'un fichier Excel"
      ...
      <NonSavedXLSXDisplayerDialog
          ...
          name="Les pieces non sauvegardés"
          loading={submitionLoading}
