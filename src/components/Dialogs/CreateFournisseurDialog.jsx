...
export default function FournisseurDialog(props) {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/fournisseur/create`, 
        { 
          fullname: Name,
        }, 
        {
      ...
      if (error.response) {
        ...
        console.error("Error creating fournisseur");
      }
    }
  };
  ...
  const onUploadXLSXFile = async (formData) => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/xlsx/upload/fournisseur`, 
        formData, 
        {
          headers: {
      ...
      if (error.response) {
        ...
        // Request was made but no response was received
        console.error("Error importing Zonnes: No response received");
      } else {
        // Something happened in setting up the request that triggered an Error
        console.error("Error importing Zonnes", error);
      }
    }
  }
  ...
  >
        ...
        <DialogContent sx={{ marginTop: '40px' }}> 
          <Box display="flex" flexDirection="column" alignItems="flex-start" mt={2} sx={{ width: '100%' }}>
            <TextFieldComponent 
                type="text" 
                label="Name" 
                initialHelperText="Entrer le nom du fournisseur" 
      ...
      <ConfirmationUploadExcelDialog
        ...
        name="d'insertion des fournisseurs à partir d'un fichier Excel"
      ...
      <NonSavedXLSXDisplayerDialog
        ...
        name="Les fournisseur non sauvegardés"
        loading={submitionLoading}
