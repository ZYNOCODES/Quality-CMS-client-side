...
export default function FamilyDialog(props) {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/family`, 
        { 
          name: Name,
        }, 
        {
      ...
      if (error.response) {
        ...
        console.error("Error creating family: No response received");
      } else {
  ...
  const onUploadXLSXFile = async (formData) => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_APP_URL_BASE+`/xlsx/upload/family`, 
        formData, 
        {
          headers: {
      ...
      if (error.response) {
        ...
        // Request was made but no response was received
        console.error("Error importing families: No response received");
      } else {
        // Something happened in setting up the request that triggered an Error
  ...
  return (
    <React.Fragment>
        ...
        <DialogContent sx={{ marginTop: '40px' }}> 
          <Box display="flex" flexDirection="column" alignItems="flex-start" mt={2} sx={{ width: '100%' }}>
            <TextFieldComponent 
                type="text" 
                label="Name" 
                initialHelperText="Entrer le nom de la famille" 
      ...
      <ConfirmationUploadExcelDialog
        ...
        name="d'insertion des familles à partir d'un fichier Excel"
      ...
      <NonSavedXLSXDisplayerDialog
        ...
        name="Les familles non sauvegardés"
        loading={submitionLoading}
