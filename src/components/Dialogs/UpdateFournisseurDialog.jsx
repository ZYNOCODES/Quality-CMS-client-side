...
export default function UpdateFournisseurDialog(props) {
    ...
    const handleOnUpdate = async (event) => {
        ...
        if (confirmation) {
            try {
                setLoading(true);
                const response = await axios.patch(import.meta.env.VITE_APP_URL_BASE+`/fournisseur/update/${props.code}`, 
                    {
                        fullname: Name
                    },
                    {
                ...
                if (error.response) {
                    ...
                    console.error("Error updating fournisseur: No response received");
                } else {
